...
// ===== Ledger Source Hint (Safari vs Chrome) =====
function detectLedgerSourceText(){
  const ua = navigator.userAgent || "";
  const isChrome = ua.includes("CriOS"); // iOS Chrome
  return isChrome ? "帳本來源：Chrome 本機" : "帳本來源：Safari 本機";
}
function renderLedgerSource(){
  const el = document.getElementById("ledgerSource");
  if(!el) return;
  el.textContent = detectLedgerSourceText();
}
document.addEventListener("DOMContentLoaded", () => {
  try{ renderLedgerSource(); }catch(e){}
});
