...
function buildChartData(log, manualPrice, planQty){
  const labels = [];
  const price = [];
  const avg = [];
  const qtyAfter = [];
  ...
  for(const r of (log?.rows || [])){
    ...
    points.push({kind:"trade", date, line2});
  }

  // optional manual "today price" point (not saved as a trade)
  const mp = toNumber(manualPrice, NaN);
  if(Number.isFinite(mp) && mp > 0){
    const d = new Date();
    const date = formatDateYYYYMMDD(d);
    labels.push(date);                 // X-axis last shows today's date
    price.push(mp);                    // today's price
    avg.push(toNumber(log?.avgCost, 0)); // keep avg cost line extended (avg doesn't change)
    qtyAfter.push(toNumber(log?.holdingQty, 0));
    points.push({kind:"today", date, line2:`現價 @${fmtNum(mp,2)}`});
  }


  // planned buy (uses manual today price)
  let planAvg = null;
  let planAvgValue = null;
  let planIndex = null;

  if(Number.isFinite(planQty) && planQty>0 && Number.isFinite(mp) && mp > 0){
    const holdQ = toNumber(log?.holdingQty, 0);
    const avg0 = toNumber(log?.avgCost, 0);
    const newQ = holdQ + planQty;
    planAvgValue = newQ>0 ? ((holdQ*avg0 + planQty*mp) / newQ) : 0;

    // place the black dot on the last x (today price point)
    planIndex = labels.length - 1;
    planAvg = new Array(labels.length).fill(null);
    planAvg[planIndex] = planAvgValue;
  }

  return {labels, price, avg, qtyAfter, points, planAvg, planAvgValue, planQty: (planAvg ? planQty : null), planIndex};
}
...
function applyAppVersionToTitle(){
  try{
    const h1 = document.querySelector("h1");
    if(!h1) return;
    const raw = (h1.textContent || "").trim();
    const base = raw.replace(/\s+v\d+(?:\.\d+)*$/i, "").trim();
    h1.textContent = `${base} ${APP_VERSION}`;
  }catch(_){}
}

function $(id){ return document.getElementById(id); }

function ensureTodayPriceUI(){
  const qSym = $("q_symbol");
  if(!qSym) return;
  if($("q_today_price")) return;

  // Query card container
  const split = qSym.closest(".split") || qSym.closest(".card") || qSym.parentElement;
  if(!split) return;

  const wrap = document.createElement("div");
  wrap.innerHTML = `
    <label>當日價格（手動輸入，可空白）</label>
    <input id="q_today_price" inputmode="decimal" placeholder="例如：602.5" />
    <div class="hint" style="margin-top:6px;">只用來顯示在圖表最後一筆「現價」，不會寫入交易、不影響庫存。</div>
    <div style="margin-top:12px;">
      <label>預估要買的股數（手動輸入，可空白）</label>
      <input id="q_plan_qty" inputmode="numeric" placeholder="例如：100" />
      <div class="hint" style="margin-top:6px;">會在圖表上新增一個「黑點」，表示用「當日價格」買入這些股數後的預估平均成本落點（不會寫入交易）。</div>
    </div>
    <div style="margin-top:10px; display:flex; gap:10px; flex-wrap:wrap;">
      <button type="button" id="btn_export_chart_a4" class="btn">匯出圖表 PNG（A4）</button>
    </div>
    <div class="hint" style="margin-top:6px;">匯出會在圖表上方加上「市場＋代號＋日期」標題，方便你保存/分享。</div>
  `;

  const btn = $("btn_query");
  const btnBox = btn ? btn.parentElement : null;

  // Insert before query button (keeps UI less crowded)
  if(btnBox && btnBox.parentElement === split){
    split.insertBefore(wrap, btnBox);
  }else{
    split.appendChild(wrap);
  }

  const inp = $("q_today_price");
  if(inp){
    const onApply = ()=>{
      if(!lastQueryLog) return;
      try{ drawChartForQuery(lastQueryLog); }catch(_){}
    };
    inp.addEventListener("change", onApply);
    inp.addEventListener("blur", onApply);
    const plan = $("q_plan_qty");
    if(plan){
      plan.addEventListener("change", onApply);
      plan.addEventListener("blur", onApply);
    }
    // Export chart as A4 PNG
    const btnExp = $("btn_export_chart_a4");
    if(btnExp){
      btnExp.addEventListener("click", ()=>{
        try{ exportQueryChartA4Png(); }catch(e){ alert("匯出失敗：" + (e?.message || e)); }
      });
    }
  }
}

function getManualTodayPrice(){
  const el = $("q_today_price");
  if(!el) return null;
...
function drawChartForQuery(log){
  const mp = getManualTodayPrice();
  const pq = getPlannedBuyQty();
  const cd = buildChartData(log, mp, pq);
  ...
  chart = new Chart(ctx, {
    ...
    data: {
      labels: cd.labels,
      datasets: [
        { label: "成交價", data: cd.price, tension: 0.25, borderColor: "#1e88e5", backgroundColor: "#1e88e5", pointBackgroundColor: "#1e88e5", pointBorderColor: "#1e88e5", pointRadius: 4, pointHoverRadius: 6 },
        { label: "平均成本", data: cd.avg, tension: 0.25, borderColor: "#ff5c93", backgroundColor: "#ff5c93", pointBackgroundColor: "#ff5c93", pointBorderColor: "#ff5c93", pointRadius: 4, pointHoverRadius: 6 },
        ...(cd.planAvg ? [{
          label: "預估買後均價",
          data: cd.planAvg,
          showLine: false,
        ...
        tooltip: {
          callbacks: {
            title: (items)=>{
              ...
              const p = cd.points[i] || {kind:"trade", date:"", line2:""};
              const q = cd.qtyAfter[i] ?? 0;

              if(p.kind === "today"){
                return [`現價  ${p.date}`, p.line2, `庫存：${fmtInt(q)}`];
              }
              const label = cd.labels[i] ?? "";
              return [`#${label}  ${p.date}`, p.line2, `庫存：${fmtInt(q)}`];
            }
          }
        },
        legend: { display: true }
        ...
        x: {
          ticks: {
            callback: (value, index) => {
              const p = cd.points[index] || {};
              if(p.kind === "today"){
                return [String(cd.labels[index] || ""), "現價"];
              }
              const idxLabel = cd.labels[index] || "";
              const q = cd.qtyAfter[index] ?? 0;
              return [String(idxLabel), `庫存 ${fmtInt(q)}`];
            }
          }
        }
      }
...
// ===== Export chart as high-resolution A4 PNG (landscape) =====
async function exportQueryChartA4Png(){
  if(!window.Chart){
    alert("找不到 Chart.js，無法匯出圖表。");
    return;
  }
  if(!lastQueryLog || !lastQueryLog.rows || !Array.isArray(lastQueryLog.rows) || lastQueryLog.rows.length === 0){
    alert("目前沒有圖表資料可匯出。請先查詢一支股票。");
    return;
  }

  const sym = (lastQuerySymbol || ($("q_symbol")?.value || "").trim() || "UNKNOWN");
  const today = new Date();
  const yyyy = today.getFullYear();
  const mm = String(today.getMonth()+1).padStart(2,"0");
  const dd = String(today.getDate()).padStart(2,"0");
  const dateStr = `${yyyy}-${mm}-${dd}`;
    const mkt = (String(lastQueryMarket || ($("q_market")?.value || "")).trim().toUpperCase() || "");
  const mkt2 = (mkt==="TW"||mkt==="US") ? mkt : (mkt || "");
  const titleText = `${mkt2 ? (mkt2 + " ") : ""}${sym} ${dateStr}`;

  // A4 landscape export target (300DPI: 3508x2480; fallback 200DPI: 2339x1654)
  // Build the same chart data currently shown (includes "現價" if you typed it)
  const mp = getManualTodayPrice();
  const pq = getPlannedBuyQty();
  const cd = buildChartData(lastQueryLog, mp, pq);

  // (canvas will be created during export)
  ...
  const headerPlugin = {
    ...
    beforeDraw(chartInstance){
      ...
      ctx.fillText(titleText, 90, 90);
      // Subtitle: export time
      ctx.font = "28px system-ui, -apple-system, Segoe UI, Roboto, Helvetica, Arial";
      const hh = String(today.getHours()).padStart(2,"0");
      const mi = String(today.getMinutes()).padStart(2,"0");
      ctx.fillStyle = "#666";
      ctx.fillText(`匯出時間：${dateStr} ${hh}:${mi}`, 90, 140);
      ctx.restore();
    }
  };

  // Export-only annotations: print BUY/SELL info next to each point (like tooltip)
  const annoPlugin = {
    id: "a4Anno",
    afterDatasetsDraw(chartInstance){
      try{
        ...
        const meta = chartInstance.getDatasetMeta(0); // dataset 0 = 成交價
        if(!meta || !meta.data || !area) return;

        const boxes = [];
        const h = chartInstance.height || 2480;
        const font1 = Math.max(16, Math.round(h / 120)); // title line
        const font2 = Math.max(14, Math.round(font1 * 0.9)); // other lines
        const lineGap = Math.round(font2 * 0.35);
        const padX = Math.round(font2 * 0.6);
        const padY = Math.round(font2 * 0.45);
        const radius = Math.round(font2 * 0.4);

        function measureLines(lines){
          let w = 0;
          for(let i=0;i<lines.length;i++){
            const isTitle = (i===0);
            ctx.font = `${isTitle ? "600" : "400"} ${isTitle ? font1 : font2}px system-ui, -apple-system, Segoe UI, Roboto, Helvetica, Arial`;
            w = Math.max(w, ctx.measureText(String(lines[i]||"")).width);
          }
          const lhTitle = font1 + lineGap;
          const lh = font2 + lineGap;
          const height = padY*2 + lhTitle + Math.max(0, (lines.length-1))*lh;
          return {w, h: height, lhTitle, lh};
        }
        ...
        for(let i=0;i<meta.data.length;i++){
          ...
          const p = cd.points[i] || {kind:"trade", date:"", line2:""};
          const q = cd.qtyAfter[i] ?? 0;
          const idxLabel = cd.labels[i] ?? "";

          let lines;
          if(p.kind === "today"){
            lines = [`${p.date} 現價`, String(p.line2||""), `庫存：${fmtInt(q)}`];
          }else{
            lines = [`#${idxLabel} ${p.date}`, String(p.line2||""), `庫存：${fmtInt(q)}`];
          }

          // tiny cleanup for readability
          lines = lines.map(s=>String(s||"").replace(/\|/g,"").replace("｜","  "));

          const ms = measureLines(lines);
          const boxW = Math.ceil(ms.w + padX*2);
          const boxH = Math.ceil(ms.h);

          const above = (i % 2 === 0);
          const offset = Math.round(font2 * 0.9);
          let x = el.x + offset;
          let y = above ? (el.y - boxH - offset) : (el.y + offset);

          // flip to left side if overflow right
          if(x + boxW > area.right - 4){
            x = el.x - boxW - offset;
          }
          ...
          let tries = 0;
          while(tries < 6 && boxes.some(b=>intersects(rect,b))){
            y = y + (above ? -(boxH + 8) : (boxH + 8));
            y = Math.max(area.top + 4, Math.min(y, area.bottom - boxH - 4));
          ...
          ctx.font = `600 ${font1}px system-ui, -apple-system, Segoe UI, Roboto, Helvetica, Arial`;
          ctx.fillText(lines[0], rect.x + padX, ty);
          ty += ms.lhTitle;

          ctx.font = `400 ${font2}px system-ui, -apple-system, Segoe UI, Roboto, Helvetica, Arial`;
          for(let li=1; li<lines.length; li++){
            ctx.fillText(lines[li], rect.x + padX, ty);
            ty += ms.lh;
          }
        }
  ...
  const cfg = {
    ...
    options: {
      responsive: false,
      animation: false,
      // extra top padding to make room for header
      layout: { padding: { top: 170, left: 70, right: 70, bottom: 160 } },
      plugins: {
        legend: { display: true },
        tooltip: { enabled: false } // export image doesn't need hover tooltip
      },
      scales: {
        x: {
  ticks: {
    ...
    autoSkip: (cd.labels.length > 12),
    maxRotation: (cd.labels.length > 12) ? 45 : 0,
    minRotation: (cd.labels.length > 12) ? 45 : 0,
    padding: 12,
    color: "#111",
    font: { size: 22 }
  },
  grid: { color: "rgba(0,0,0,0.08)" }
    ...
    plugins: [bgPlugin, headerPlugin, annoPlugin]
  };

  
  // Render to blob (A4 300DPI first; fallback to 200DPI if iOS fails toBlob)
  const market = normalizeMarket(($("q_market")?.value || lastQueryMarket || "").toString());
  const filename = `stock-ledger_chart_${market}_${sym}_${yyyy}${mm}${dd}.png`;
  ...
  async function canvasToBlobSafe(canvas){
    ...
    // 2) fallback toDataURL -> Blob (some iOS builds return null for toBlob)
    try{
  ...
  async function renderAndGetBlob(width, height){
    const c = document.createElement("canvas");
    c.width = width;
    c.height = height;

    const exportChart = new Chart(c.getContext("2d"), cfg);
    await new Promise(r=>setTimeout(r, 80)); // give iOS a bit more time

    const blob = await canvasToBlobSafe(c);
    exportChart.destroy();
    return blob;
  }

  // A4 landscape @ 300DPI: 3508x2480
  let blob = await renderAndGetBlob(3508, 2480);

  // If failed (common on iOS PWA due to memory), fallback to 200DPI: 2339x1654
  if(!blob){
    blob = await renderAndGetBlob(2339, 1654);
  }

  if(!blob){
    alert("匯出失敗：無法產生圖片（可能是 iOS 記憶體限制）。建議先截圖，或我可以再加一個「匯出較小尺寸」按鈕。");
    return;
  }
// iOS: share if possible, otherwise open preview in a new tab
  ...
  // Fallback: open in new tab for save/share
  const url = URL.createObjectURL(blob);
  const w = window.open(url, "_blank");
  // In case popup blocked, show a manual link
  if(!w){
    const a = document.createElement("a");
    a.href = url;
    a.target = "_blank";
    a.textContent = "點我開啟匯出圖片";
    a.style.display = "block";
    a.style.marginTop = "10px";
    const host = $("page_query") || document.body;
    host.prepend(a);
    alert("已產生圖片，請點頁面上的「點我開啟匯出圖片」來保存。");
  }
  // release later
...
(async ()=>{
  ...
  try{ applyAppVersionToTitle(); }catch(_){ }
  ...
  try{ ensureTodayPriceUI(); }catch(_){ }
  try{ renderBackupInfo(); }catch(_){ }
})();
