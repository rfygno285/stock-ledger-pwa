...
async function hydrateFromIDBIfNeeded(){
  try{
    ...
    if(!obj || !Array.isArray(obj.lots) || obj.lots.length===0) return;

    localStorage.setItem(STORAGE_KEY, JSON.stringify(obj));
    ledger = obj;

    try{ refreshHoldings(); }catch(_){}
    try{ setLedgerSourceLabel(); }catch(_){}
  }catch(_){}
}
...
function setLedgerSourceLabel(){
  const ua = navigator.userAgent || "";
  const isChrome = ua.includes("CriOS");
  const el = document.getElementById("ledgerSource");
  if(!el) return;
  const lots = Array.isArray(ledger?.lots) ? ledger.lots.length : 0;
  el.textContent = (isChrome ? "帳本來源：Chrome 本機" : "帳本來源：Safari 本機") + `（交易 ${lots} 筆）`;
}
...
try{ refreshHoldings(); }catch(_){ }
try{ setLedgerSourceLabel(); }catch(_){ }
try{ hydrateFromIDBIfNeeded(); }catch(_){ }
