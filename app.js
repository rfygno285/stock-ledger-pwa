...
function getStorageSourceLabel(){
  ...
  else if(/Safari\//.test(ua) && !/CriOS\//.test(ua) && !/FxiOS\//.test(ua) && !/EdgiOS\//.test(ua)) browser = "Safari";
  return (isStandalone ? "主畫面(PWA)" : browser) + " 本機";
}
function renderStorageSource(){
  const el = document.getElementById("storage_source");
  if(!el) return;
  el.textContent = "儲存來源：" + getStorageSourceLabel();
}
...
$("btn_import_merge")?.addEventListener("click", ()=>{
  importMode = "merge";
  $("file_import").click();
});
$("btn_import_replace")?.addEventListener("click", ()=>{
  importMode = "replace";
  $("file_import").click();
});
...
$("file_import").addEventListener("change", async (e)=>{
  ...
  try{
    ...
    const incoming = {version:1, lots: Array.isArray(obj.lots)?obj.lots:[]};
    if(importMode === "replace"){
      localStorage.setItem(STORAGE_KEY, JSON.stringify({version:1, lots: incoming.lots}));
    }else{
      // merge: union by id, fallback by content signature
      const seen = new Map();
      for(const r of (current.lots||[])){
        const id = r?.id;
        if(id) seen.set(String(id), true);
      }
      const sig = (r)=>[
        r?.timestamp||"", r?.market||"", r?.symbol||"", r?.type||"",
        String(r?.qty||""), String(r?.price||""), String(r?.fee||"")
      ].join("|");
      const sigSeen = new Set((current.lots||[]).map(sig));
      let added = 0;
      for(const r0 of incoming.lots){
        const r = (r0 && typeof r0==="object") ? {...r0} : null;
        if(!r) continue;
        if(!r.id) r.id = uuid();
        const id = String(r.id);
        const s = sig(r);
        if(seen.has(id) || sigSeen.has(s)) continue;
        current.lots.push(r);
        seen.set(id,true);
        sigSeen.add(s);
        added++;
    ...
    ledger = loadLedger();
    alert(importMode==="replace" ? "匯入完成（覆蓋）" : "匯入完成（合併）");
    refreshHoldings();
  }catch(err){
