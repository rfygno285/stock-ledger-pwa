...
function ensureTodayPriceUI(){
  ...
  wrap.innerHTML = `
    ...
    <div class="hint" style="margin-top:6px;">匯出會在圖表上方加上「代號＋日期」標題，方便你保存/分享。</div>
  `;
...
async function exportQueryChartA4Png(){
  ...
  if(!lastQueryLog || !Array.isArray(lastQueryLog) || lastQueryLog.length === 0){
    alert("目前沒有圖表資料可匯出。請先查詢一支股票。");
    return;
  }

  const sym = ( ($("q_symbol")?.value || "").trim() ) || "UNKNOWN";
  const today = new Date();
  const yyyy = today.getFullYear();
  const mm = String(today.getMonth()+1).padStart(2,"0");
  const dd = String(today.getDate()).padStart(2,"0");
  const dateStr = `${yyyy}-${mm}-${dd}`;
  const titleText = `${sym}  ${dateStr}`;
